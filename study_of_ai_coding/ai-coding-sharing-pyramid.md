...
#### skills
Anthropic定义的skills就是智能体可以识别的可复用资产。
资源的实现方式是文件：指令、脚本和资源组成的文件夹。可读性好，方便分享。
集成的实现是agentic + 沙箱。简单，通用。
体现的载体是由，以及agent具备使用bash工具的能力。
特点：
1. 按需加载（渐进式披露）。从上下文角度看就是分段按需懒加载。从检索角度看就是分级索引检索。
2. 实现简洁。从内容和集成，都更加通用，灵活，简约。
3. 相对依赖于更好的agentic能力

也提现了，沙箱系统十分契合当前阶段的模型能力、成本、可靠性之间的平衡点。


编程是非常广泛的工程领域，并且成果相对容易验证，领域方法论相对成熟，是skill应用最多的领域。所以在这个领域的社区里后面的具体实践中能够看到社区中大量的skills应用。
...
多角度评审
架构，安全，简洁代码等等角度
从不同角度，产生明确，正确的计划，作为执行标准和约束
就像六顶思考帽，指导人从多角度思考问题。多角度的评审和反问，也给token预测提供更多正确的可能性。
